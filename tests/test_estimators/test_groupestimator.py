...
def test_estimator(simple_dataframe):
    # test grouping
    x, y = GroupEstimator(DummyClassifier(), "col_1").separate_dataset(
            simple_dataframe, simple_dataframe["Class"], "col_1")
    assert len(x) == 2 and len(y) == 2

    # test grouping with model
    res = GroupEstimator(DummyClassifier(), "col_1").fit(
            simple_dataframe, simple_dataframe["Class"]).predict(simple_dataframe)
...
def test_shape_regression(estimator, random_xy_dataset_regr):
    ...
    X_len = len(X[0])
    X = pd.DataFrame(
            X, columns=[str(i) for i in range(X_len)])
